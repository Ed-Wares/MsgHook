// MsgLookup.h
// This file contains a lookup table for Windows Message names.

// MSG Array LOOKUP
const int MAX_MSG_LOOKUP = 1024;
const int MAX_MSG_NAME = 21;
TCHAR MSG_LOOKUP[MAX_MSG_LOOKUP][MAX_MSG_NAME] = { 0 }; // Array to hold message names
...
void InitializeMsgLookup(int allowList[], int allowSize) 
{
	for (int i = 0 ; i < MAX_MSG_LOOKUP ; i++)
	{
		...
		switch (i)
		{
			...
			default:
				memset((void *)&MSG_LOOKUP[i], '\0', sizeof(TCHAR) * (MAX_MSG_NAME - 1)); //blank it
...
void GetMsgNameFromMsgId(UINT msgId, TCHAR* outName, size_t nameSize)
{
	if (msgId < MAX_MSG_LOOKUP && MSG_LOOKUP[msgId][0] != '\0')
	{
		_tcscpy_s(outName, nameSize, MSG_LOOKUP[msgId]);
	}
	else
	{
		_stprintf_s(outName, nameSize, _T("MSG_0x%04X"), msgId);
	}
}